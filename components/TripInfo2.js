...
//page to view additional information for a pending trip and either accept or decline the trip
...
export default class TripInfo2 extends React.Component {
  ...
  tripAccept(){//function to accept a trip and sed driver information to database on the assigned trip
    
    axios.post(route + '/acceptTrip', {user_id: user_id, idtrip: this.props.navigation.state.params.trip.idtrip})
          .then(res =>{
            console.log(res);
            console.log(res.data);
            if (res.data == 1){
                Alert.alert(//alert user of successful acceptance
                  "Trip Accepted!",
                  "You have been added as the driver to this trip.",
                  [
                ...
                Alert.alert(//alert user of failure
                  "Submission Failed!",
                  "You have not been added as the driver to this trip. Please reload the page to make sure the trip is still available.",
                  [
                    { text: "OK",}
                  ],
                  { cancelable: false }
    ...
    tripDecline(){//function to decline a trip and send driver information to database for that trip
    
      axios.post(route + '/declineTrip', {user_id: user_id, idtrip: this.props.navigation.state.params.trip.idtrip})
            .then(res =>{
              console.log(res);
              console.log(res.data);

                  Alert.alert(//alert user that they have declined the trip
                    "Trip Declined!",
                    "You have marked that you are unable to take this trip.",
                    [
                      { text: "OK", onPress: () => this.props.navigation.navigate('Home2')}
                    ],
                    { cancelable: false }
    ...
    <View style={styles.buttonContainer}> 
        <TouchableOpacity onPress={() => this.tripAccept()}//on press try tripaccept function
        style={styles.button}
         >
         <Text style={styles.buttonText}>Accept
         </Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => this.tripDecline()}//on press try tripdecline function
        style={styles.button}
         >
         <Text style={styles.buttonText}>Decline
         </Text>
        </TouchableOpacity>
    </View>
    </ScrollView>
